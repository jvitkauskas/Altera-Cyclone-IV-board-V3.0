...
void LCD_WR_DATA16(unsigned int data)//写积存器数据
{
     ...
     LCD_CS_L(); 
      ...
      GPIOA->ODR=(GPIOA->ODR&0XFF00)| (unsigned char)(data>>8);
     LCD_WR_L();
     LCD_WR_H();

     GPIOA->ODR=(GPIOA->ODR&0XFF00)|(unsigned char)data;     
     LCD_WR_L();
     LCD_WR_H();
...
{   
        ...
        LCD_RST_H();
         ...
         LCD_WR_REG(0x01, 0x0100); // set SS and SM bit
...
/*		 
         ...
         LCD_WR_REG(0x11, 0x0307); // DC1[2:0], DC0[2:0], VC[2:0]
          Delay(10); // Delay 50ms
        
         LCD_WR_REG(0x12, 0x0095); // Internal reference voltage= Vci;
         Delay(10); // Delay 50ms
         LCD_WR_REG(0x13, 0x1500); // Set VDV[4:0] for VCOM amplitude
         LCD_WR_REG(0x29, 0x0016); // Set VCM[5:0] for VCOMH
         ...
         LCD_WR_REG(0x29, 0x0025); // Set VCM[5:0] for VCOMH
         LCD_WR_REG(0x2B, 0x000c); // Set Frame Rate
         Delay(10); // Delay 50ms  */  
          Delay(50); // Dis-charge capacitor power voltage
         ...
         LCD_WR_REG(0x12, 0x001d); // Internal reference voltage= Vci;
         Delay(10); // Delay 50ms
