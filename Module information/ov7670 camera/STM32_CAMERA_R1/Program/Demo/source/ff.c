...
/*-----------------------------------------------------------------------*/
/* Mount/Unmount a Logical Drive      增加一个逻辑驱动器                 */
/*-----------------------------------------------------------------------*/

FRESULT f_mount (
	BYTE vol,		/* Logical drive number to be mounted/unmounted     被增加的逻辑驱动器的号码*/
	FATFS *fs		/* Pointer to new file system object (NULL for unmount)   指向新的文件系统*/
)
{
	FATFS *rfs;
...
/*-----------------------------------------------------------------------*/
/* Open or Create a File      打开或创建一个文件                         */
/*-----------------------------------------------------------------------*/

FRESULT f_open (
	FIL *fp,			/* Pointer to the blank file object   指向空目标文件*/
	const XCHAR *path,	/* Pointer to the file name    指向文件名*/
	BYTE mode			/* Access mode and file open mode flags 进入模式和文件打开模式标志*/
)
{
	FRESULT res;
	DIR dj;
...
FRESULT f_read (
	FIL *fp, 		/* Pointer to the file object 指向目标文件*/
	void *buff,		/* Pointer to data buffer   指向数据缓冲器*/
	UINT btr,		/* Number of bytes to read   读取的位数*/
	UINT *br		/* Pointer to number of bytes read 指向读取的位数*/
)
{
	FRESULT res;
...
/*-----------------------------------------------------------------------*/
/* Synchronize the File Object     使文件同步                            */
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Change Current Drive/Directory        改变通用驱动器或目录            */
/*-----------------------------------------------------------------------*/
...
#if _FS_MINIMIZE <= 1
/*-----------------------------------------------------------------------*/
/* Create a Directory Object       创建目录                              */
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Read Directory Entry in Sequence       顺序读取目录条目               */
/*-----------------------------------------------------------------------*/
...
#if !_FS_READONLY
/*-----------------------------------------------------------------------*/
/* Get Number of Free Clusters       获取空闲簇的数目                    */
/*-----------------------------------------------------------------------*/
